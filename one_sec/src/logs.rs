...
impl Sink for PrintProxySink {
    fn append(&self, entry: ic_canister_log::LogEntry) {
        if self.0 == "ERROR" {
            ic_cdk::println!("{}:{} {}", entry.file, entry.line, entry.message);
        }
        self.1.append(entry)
    }
}
...
impl Log {
    pub fn push_logs(&mut self, priority: Priority) {
        let logs = match priority {
            ...
            Priority::Http => export_logs(&TRACE_HTTP_BUF),
        };
        for entry in logs {
    ...
    pub fn pretty(&self, max_body_size: usize) -> String {
        ...
        for entry in self.entries.iter() {
            let secs = entry.timestamp / NS_PER_SECOND;
            let x = secs % SECONDS_PER_DAY;
            let h = x / SECONDS_PER_HOUR;
            let m = (x % SECONDS_PER_HOUR) / 60;
            let s = x % 60;
            let line = format!("{:02}:{:02}:{:02}: {}", h, m, s, entry.message);
            size += line.len() + 50;
    ...
    pub fn sort_asc(&mut self) {
        self.entries.sort_by(|a, b| a.counter.cmp(&b.counter));
    }

    pub fn sort_desc(&mut self) {
        self.entries.sort_by(|a, b| b.counter.cmp(&a.counter));
    }
}
