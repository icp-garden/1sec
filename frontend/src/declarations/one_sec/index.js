export const idlFactory = ({ IDL }) => {
	...
	return IDL.Service({
		...
		get_forwarding_transactions: IDL.Func([EvmChain], [IDL.Vec(SignedForwardingTx)], ['query']),
		get_icp_token_metadata: IDL.Func([], [IDL.Vec(IcpTokenMetadata)], ['query']),
		...
		get_transfer: IDL.Func([TransferId], [IDL.Variant({ Ok: Transfer, Err: IDL.Text })], ['query']),
		get_transfer_fees: IDL.Func([], [IDL.Vec(TransferFee)], ['query']),
		get_transfer_stats: IDL.Func([IDL.Nat64], [IDL.Vec(TransferStats)], ['query']),
		...
		transfer_evm_to_icp: IDL.Func([TransferEvmToIcpArg], [TransferResponse], []),
		transfer_icp_to_evm: IDL.Func([TransferIcpToEvmArg], [TransferResponse], []),
		validate_forwarding_address: IDL.Func([IcpAccount, IDL.Text], [Result], ['query'])
	});
};
